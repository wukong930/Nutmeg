...
type HistoricalProbabilityCalibrationProfileRuntimeReplayStatus = Literal[
...
type HistoricalProbabilityCalibrationProfileRuntimeReplayCheckStatus = Literal[
    "passed",
    "failed",
    "skipped",
]
...
class ProbabilityCalibrationRuntimeProfileSet(BaseModel):
    profile_version: str = "unknown"
    calculation_basis: str = "probability_calibration_profile_runtime_loader_v3_1"
    ...
    profiles: list[CandidateProbabilityCalibrationProfile] = Field(default_factory=list)
    profile_proposals_json: list[dict[str, object]] = Field(default_factory=list)
    source_json_path: str | None = None
    notes: list[str] = Field(default_factory=list)


class HistoricalProbabilityCalibrationProfileRuntimeReplayOptions(BaseModel):
    ...
    require_no_internal_strategy_label_exposure: bool = True
    max_selected_profile_count: int = Field(default=1, ge=1)


class HistoricalProbabilityCalibrationProfileRuntimeReplayCheck(BaseModel):
    name: str
    status: HistoricalProbabilityCalibrationProfileRuntimeReplayCheckStatus
    actual: float | int | str | bool | None = None
    threshold: float | int | str | bool | None = None
    detail: str


class HistoricalProbabilityCalibrationProfileRuntimeReplayReport(BaseModel):
    report_key: str
    status: HistoricalProbabilityCalibrationProfileRuntimeReplayStatus
    runtime_replay_allowed: bool
    holdout_replay_allowed: bool
    source_profile_version: str
    profile_count: int = Field(ge=0)
    selected_profile_count: int = Field(ge=0)
    ...
    internal_strategy_label_exposed: bool = False
    checks: list[HistoricalProbabilityCalibrationProfileRuntimeReplayCheck] = (
        Field(default_factory=list)
    )
    profile_set_json: dict[str, object] = Field(default_factory=dict)
    selected_profile_json: dict[str, object] | None = None
...
class _CalibrationReplayInput(BaseModel):
    slices: list[HistoricalRecommendationSlice] = Field(default_factory=list)
    adjusted_fixture_count: int = Field(default=0, ge=0)
    adjusted_candidate_count: int = Field(default=0, ge=0)
    skipped_group_count: int = Field(default=0, ge=0)
...
def load_probability_calibration_runtime_profile_set(
    path: Path | str,
    *,
    enable_shadow_replay: bool = False,
    ...
    profiles = [
        CandidateProbabilityCalibrationProfile.model_validate(profile)
        for profile in _mapping_list(
            profile_json.get("candidate_probability_calibration_profiles")
        )
    ]
    if not profiles:
        profiles = [
            CandidateProbabilityCalibrationProfile.model_validate(profile)
            for profile in _mapping_list(profile_json.get("profiles"))
        ]
    if not profiles and isinstance(profile_json.get("profile_key"), str):
        profiles = [CandidateProbabilityCalibrationProfile.model_validate(profile_json)]
    return ProbabilityCalibrationRuntimeProfileSet(
        profile_version=_string(profile_json.get("profile_version")) or "unknown",
        calculation_basis=(
            _string(profile_json.get("calculation_basis"))
            or "probability_calibration_profile_runtime_loader_v3_1"
        ),
        ...
        profiles=profiles,
        profile_proposals_json=_mapping_list(
            profile_json.get("candidate_probability_calibration_profile_proposals")
        ),
...
def build_historical_probability_calibration_profile_runtime_replay_report(
    historical_slices: Sequence[HistoricalRecommendationSlice],
    *,
    profile_set: ProbabilityCalibrationRuntimeProfileSet,
    options: (
        HistoricalProbabilityCalibrationProfileRuntimeReplayOptions | None
    ) = None,
) -> HistoricalProbabilityCalibrationProfileRuntimeReplayReport:
    resolved_options = (
        options or HistoricalProbabilityCalibrationProfileRuntimeReplayOptions()
    )
    resolved_profile_set = profile_set.model_copy(
        update={"shadow_replay_enabled": resolved_options.enable_shadow_replay}
    )
    selected_profiles = _selected_profiles(
        resolved_profile_set,
        options=resolved_options,
    )
    warnings: list[str] = []
    if not resolved_options.enable_shadow_replay:
        warnings.append(
            "probability_calibration_profile_runtime_replay:disabled_by_feature_flag"
        )
    ...
    if not selected_profiles:
        warnings.append(
            "probability_calibration_profile_runtime_replay:no_selected_profiles"
        )
        return _empty_report(
            resolved_profile_set,
            selected_profiles=selected_profiles,
    ...
    if len(selected_profiles) > resolved_options.max_selected_profile_count:
        warnings.append(
            "probability_calibration_profile_runtime_replay:too_many_selected_profiles"
        )
        checks = [
            _maximum_check(
                name="selected_profile_count",
                actual=len(selected_profiles),
                threshold=resolved_options.max_selected_profile_count,
                detail="runtime replay currently supports a bounded profile set",
            )
        ]
        return _empty_report(
            resolved_profile_set,
    ...
    calibration_input = _calibrated_replay_input(
        historical_slices,
        profile=selected_profile,
    )
    warnings.extend(calibration_input.warning_codes)
    ...
    runtime_allowed = all(check.status != "failed" for check in checks)
    ...
    summary: dict[str, object] = {
        "calculation_basis": (
            "historical_probability_calibration_profile_runtime_replay_v3_1"
        ),
        "status": status,
        "runtime_replay_allowed": runtime_allowed,
        "holdout_replay_allowed": holdout_allowed,
        "source_profile_version": resolved_profile_set.profile_version,
        "source_profile_status": resolved_profile_set.status,
        "profile_count": len(resolved_profile_set.profiles),
        "selected_profile_count": len(selected_profiles),
        "selected_profile_key": selected_profile.profile_key,
        "selected_profile_mode": selected_profile.mode,
        "selected_profile_segment_mode": selected_profile.segment_mode,
        "baseline_suite_key": baseline_suite.suite_key,
        "candidate_suite_key": candidate_suite.suite_key,
        "adjusted_fixture_count": calibration_input.adjusted_fixture_count,
        "adjusted_candidate_count": calibration_input.adjusted_candidate_count,
        "skipped_group_count": calibration_input.skipped_group_count,
    ...
    report_key = _report_key(
        summary,
        checks,
        profile_set=resolved_profile_set,
        selected_profile=selected_profile,
    )
    return HistoricalProbabilityCalibrationProfileRuntimeReplayReport(
        ...
        improvement_count_vs_baseline=metrics.improvement_count_vs_baseline,
        checks=checks,
        profile_set_json=resolved_profile_set.model_dump(mode="json"),
        selected_profile_json=selected_profile.model_dump(mode="json"),
        warnings=warnings,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    report = build_historical_probability_calibration_profile_runtime_replay_report(
        loaded_slices.slices,
        profile_set=load_probability_calibration_runtime_profile_set(
            args.profile_set,
...
def _selected_profiles(
    profile_set: ProbabilityCalibrationRuntimeProfileSet,
    *,
    options: HistoricalProbabilityCalibrationProfileRuntimeReplayOptions,
) -> list[CandidateProbabilityCalibrationProfile]:
    profile_keys = set(options.profile_keys)
    profiles = [
        profile
        for profile in profile_set.profiles
        if not profile_keys or profile.profile_key in profile_keys
    ]
    return sorted(profiles, key=lambda profile: profile.profile_key)


def _calibrated_replay_input(
    historical_slices: Sequence[HistoricalRecommendationSlice],
    *,
    profile: CandidateProbabilityCalibrationProfile,
) -> _CalibrationReplayInput:
    adjusted_fixture_count = 0
    adjusted_candidate_count = 0
    skipped_group_count = 0
    warnings: list[str] = []
    calibrated_slices: list[HistoricalRecommendationSlice] = []
    competition_season_index_by_slice_id = (
        build_historical_competition_season_index_by_slice_id(historical_slices)
    )
    for historical_slice in historical_slices:
        result = apply_candidate_probability_calibration_profile(
            _candidates_from_fixtures(
                ...
                competition_season_index=(
                    competition_season_index_by_slice_id.get(
        ...
        adjusted_fixture_count += result.adjusted_fixture_count
        adjusted_candidate_count += result.adjusted_candidate_count
        skipped_group_count += result.skipped_group_count
        warnings.extend(result.warning_codes)
        calibrated_by_identity = {
            _candidate_identity(candidate): candidate
            for candidate in result.candidates
            if (
                candidate.metadata_json.get(
                    "candidate_probability_calibration_profile_key"
                )
                == profile.profile_key
            )
        }
            ...
            historical_slice.model_copy(
                update={
                    "fixtures": [
                        _calibrated_fixture(
                            fixture,
                            calibrated_by_identity=calibrated_by_identity,
                        )
    ...
    return _CalibrationReplayInput(
...
def _calibrated_fixture(
    ...
    *,
    calibrated_by_identity: Mapping[
...
def _calibrated_prediction(
    ...
    *,
    ...
    candidate = calibrated_by_identity.get(_prediction_identity(fixture, prediction))
    if candidate is None:
        return prediction
    return prediction.model_copy(
        update={
            "probability": candidate.probability,
            "model_edge": candidate.model_edge,
            "metadata_json": {
...
def _suite_pair_metrics(
    ...
    for slice_id in sorted(set(baseline_by_slice) & set(candidate_by_slice)):
        ...
        if _final_answer_signature(baseline_result.final_answer) != (
            _final_answer_signature(candidate_result.final_answer)
        ):
            changed_count += 1
...
def _checks(
    ...
    *,
    ...
    calibration_input: _CalibrationReplayInput,
    metrics: _SuitePairMetrics,
    options: HistoricalProbabilityCalibrationProfileRuntimeReplayOptions,
) -> list[HistoricalProbabilityCalibrationProfileRuntimeReplayCheck]:
    return [
        _boolean_check(
        ...
        _maximum_check(
            name="selected_profile_count",
            actual=selected_profile_count,
            threshold=options.max_selected_profile_count,
            detail="runtime replay currently supports a bounded profile set",
        ),
        _boolean_check(
            name="profile_runtime_allowed",
            actual=profile_set.runtime_profile_proposal_allowed,
            expected=True,
            enabled=options.require_profile_runtime_allowed,
            detail="source profile set must already be runtime proposal allowed",
        ),
        _boolean_check(
            name="profile_set_holdout_candidate_enabled",
            actual=profile_set.holdout_candidate_allowed,
            expected=True,
            enabled=options.require_holdout_candidate_enabled,
            detail="source profile set must be enabled for holdout replay",
        ),
        _boolean_check(
            name="profile_set_proposed_production_enabled",
            actual=profile_set.runtime_profile_proposal_allowed,
            expected=True,
            enabled=options.require_proposed_production_enabled,
            detail="source profile set must be proposed for production replay",
        ),
        _boolean_check(
            name="profile_set_no_production_change",
            actual=not profile_set.production_recommendation_changed,
            expected=True,
            enabled=options.require_no_production_change,
            detail="profile set artifact must not mark production recommendations changed",
        ),
        _boolean_check(
            name="selected_profile_active",
            actual=selected_profile.mode == "active",
            expected=True,
            enabled=options.require_active_profile,
            detail="selected profile should be active for runtime replay",
        ),
        _minimum_check(
            name="adjusted_fixture_count",
            actual=calibration_input.adjusted_fixture_count,
            threshold=options.min_adjusted_fixture_count,
            detail="runtime replay should exercise profile-adjusted fixtures",
        ),
        _minimum_check(
            name="adjusted_candidate_count",
            actual=calibration_input.adjusted_candidate_count,
            threshold=options.min_adjusted_candidate_count,
            detail="runtime replay should exercise profile-adjusted candidates",
        ),
        _minimum_check(
        ...
        _boolean_check(
            name="internal_strategy_label_not_exposed",
            actual=True,
            expected=True,
            enabled=options.require_no_internal_strategy_label_exposure,
            detail="runtime replay must not expose internal strategy labels",
        ),
    ]
...
def _empty_report(
    profile_set: ProbabilityCalibrationRuntimeProfileSet,
    *,
    selected_profiles: Sequence[CandidateProbabilityCalibrationProfile],
    checks: Sequence[HistoricalProbabilityCalibrationProfileRuntimeReplayCheck],
    status: HistoricalProbabilityCalibrationProfileRuntimeReplayStatus,
    ...
    summary: dict[str, object] = {
        ...
        "source_profile_status": profile_set.status,
        "profile_count": len(profile_set.profiles),
        "selected_profile_count": len(selected_profiles),
        "selected_profile_keys": [profile.profile_key for profile in selected_profiles],
        "production_recommendation_changed": False,
        "public_response_changed": False,
        "internal_strategy_label_exposed": False,
    ...
    report_key = _report_key(
        ...
        selected_profile=selected_profiles[0] if len(selected_profiles) == 1 else None,
    )
    return HistoricalProbabilityCalibrationProfileRuntimeReplayReport(
        report_key=report_key,
        status=status,
        ...
        selected_profile_key=(
            selected_profiles[0].profile_key if len(selected_profiles) == 1 else None
        ),
        checks=list(checks),
        profile_set_json=profile_set.model_dump(mode="json"),
        selected_profile_json=(
            selected_profiles[0].model_dump(mode="json")
...
) -> HistoricalProbabilityCalibrationProfileRuntimeReplayStatus:
    if runtime_allowed:
...
def _holdout_checks_passed(
    ...
    ignored_for_holdout = {
        ...
        "profile_set_proposed_production_enabled",
    }
    return all(
        check.status != "failed"
        for check in checks
...
def _warnings(
    *,
    ...
    if status == "holdout_replay_passed":
        warnings.append("probability_calibration_profile_runtime_replay:holdout_only")
    elif status == "shadow_replay_failed":
        warnings.append("probability_calibration_profile_runtime_replay:failed")
    for check in checks:
        if check.status == "failed":
            warnings.append(
                "probability_calibration_profile_runtime_replay:failed_check:"
                f"{check.name}"
            )
    return warnings
...
def _extract_profile_json(payload: Mapping[str, object]) -> dict[str, object]:
    refined_profile = payload.get("refined_profile_set_json")
    if isinstance(refined_profile, Mapping):
        return dict(refined_profile)
...
def _boolean_check(
    *,
    ...
    detail: str,
    enabled: bool = True,
) -> HistoricalProbabilityCalibrationProfileRuntimeReplayCheck:
    if not enabled:
        return HistoricalProbabilityCalibrationProfileRuntimeReplayCheck(
            name=name,
            status="skipped",
            actual=actual,
...
def _parse_args(argv: Sequence[str] | None = None) -> Namespace:
    parser = ArgumentParser(
        description="Replay probability calibration profile artifacts in shadow."
    )
    ...
    parser.add_argument("--allow-disabled-holdout-profile", action="store_true")
    ...
    parser.add_argument("--allow-internal-strategy-label-exposure", action="store_true")
    parser.add_argument("--max-selected-profile-count", type=int, default=1)
...
def _options_from_args(
    ...
    return HistoricalProbabilityCalibrationProfileRuntimeReplayOptions(
        ...
        require_holdout_candidate_enabled=not args.allow_disabled_holdout_profile,
        ...
        require_no_internal_strategy_label_exposure=(
            not args.allow_internal_strategy_label_exposure
        ),
        max_selected_profile_count=args.max_selected_profile_count,
    )
...
def _report_key(
    ...
    *,
    profile_set: ProbabilityCalibrationRuntimeProfileSet,
    selected_profile: CandidateProbabilityCalibrationProfile | None,
) -> str:
    payload = {
        "summary": summary,
    ...
    return f"historical_probability_calibration_profile_runtime_replay:{digest}"
