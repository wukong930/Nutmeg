...
_EN_TO_CN: dict[str, str] = {
    ...
    "EFL_CUP": "英联赛杯",
    # 补英锦标赛(2026-09-21 owner)。中文写法**实证**自档案,不是意译:
    # `jingcai_odds_history` 里含「锦标」的 league_cn **只有**「英锦标赛」1041 行
    # (league_id=86);`crown_close_history` 另有 12 行,同一写法。
    # ⭐ 意译会得到「英格兰足球联赛锦标赛」/「英格兰挑战杯」,都对不上 ——
    #    同「绝不照英文猜译名」那条红线的联赛版。
    "EFL_TROPHY": "英锦标赛",
    ...
    # 补亚冠精英(2026-09-14 owner 授权注册进市场模式)。中文写法**实证**自
    # `jingcai_odds_history.league_cn`:「亚冠精英」3435 行(另有「亚冠乙」565 行,
    # 是 AF id=18 的另一项赛事,本条不含)。
    # ⭐ 这正是下面 `_NON_DOMESTIC_CN` 里那条 2026-08-17 注释预告的情形 ——
    #   它当时写着「它没有对应的 EN 键(我们不采集这项赛事)⇒ 只在中文轨登记;
    #   **若将来 EN 轨也出现,`CUP_COMPETITIONS` 要同步注册**」。现在 EN 轨出现了,
    #   `CUP_COMPETITIONS["AFC_CL_ELITE"]` 已同步注册为 club_cup。
...
_NON_DOMESTIC_CN: frozenset[str] = frozenset({
    ...
    "英联赛杯",
    # 同类第三条(2026-09-21):英锦标赛(EFL Trophy)同样是**国内俱乐部杯赛**
    # ⇒ 不进 δ 的拟合人口。
    # 🚨 这一条是接线时**护栏当场抓出来的**,不是我记得:只加 `_EN_TO_CN["EFL_TROPHY"]`
    #    之后 `classify_league("英锦标赛")` 从 `unknown` 变成 **`domestic`** ——
    #    一个杯赛就要混进国内联赛人口了。与 2026-09-16 亚冠乙那次**逐字同形**,
    #    而那次的教训写在注释里并没有阻止我今天再踩一次;阻止我的是腿 4 那条断言。
    # ⚠️ 它还参赛 16 支 U21 学院队 —— 就算不算「杯赛」这一条,也不该进拟合人口。
    "英锦标赛",
    ...
    "德超杯",
    # 同类第十/十一条(2026-09-15)。亚运男/女足 = **国家队赛事**(亚运会),
    # 不是国内俱乐部联赛 ⇒ 不进 δ 的拟合人口(δ 拟合在各国**联赛** CSV 上),
    # 与 世界杯/欧预赛 同理。
    # 证据(不是照名字猜):`jingcai_sp` 里「亚运女足」那行是
    # `China W` vs `Hong Kong W` —— **两支国家女足**,不可能是任何一国的国内联赛。
    # 「亚运男足」目前库里 0 行(队名 2026-09-15 才补进词典),**防御性**一起加:
    # AF 侧同赛事是 `Asian Games` 的 14 场 **U23 国家队**,同一性质。
    # ⚠️ 起因是体检 `联赛标签` 报「标签表不认识 ['亚运女足']」—— 而那是**我自己**
    #    前一天补完队名后才出现的:名字解得出 ⇒ 行写进 jingcai_sp ⇒ 标签表才看见它。
    #    ⭐ 修好一层会掀开下一层,这是第二次(同 亚冠精英 2026-08-17)。
    # ⚠️ 它们**没有**对应的 EN 键(不采集这两项赛事)⇒ 只在中文轨登记,同「亚冠精英」;
    #    若将来 EN 轨也出现,`CUP_COMPETITIONS` 要同步注册。
    "亚运女足",
    "亚运男足",
