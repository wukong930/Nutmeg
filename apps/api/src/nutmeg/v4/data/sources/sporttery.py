...
_ZH_OVERRIDES: dict[str, str] = {
    # 2026-08-11 横幅点名的 4 条 —— **实时竞彩源用的写法比历史档案更长**。
    # ⭐ 这是 2026-08-09 那批(从 `jingcai_odds_history` 锚出来的)没覆盖到的一层:
    # 走势档案写「里独立 / 波特诺 / 巴黎圣曼」,而实时源写全称。同一支队两种长度,
    # 两边都得在表里 —— 只补一边,横幅会在另一条链上继续点名。
    #
    # 锚(全部**同场唯一**,不是按音猜):
    #   · 弗鲁米嫩塞(=Fluminense,已解) vs ? → 08-11 22:00Z 解放者杯唯一一场 Fluminense 主
    #   · 帕尔梅拉斯(=Palmeiras,已解)  vs ? → 08-12 22:00Z 唯一一场 Palmeiras 主
    #   · ? vs 阿斯顿维拉(=Aston Villa,已解) → 08-12 19:00Z 欧超杯**一年只有一场**
    #   · 亚拉腊 → 与「采列」同场,锚是开球时刻 18:15Z(见 team_name_zh.py 那条)
    # ⭐ 前三条与 2026-08-09 从历史档案锚出的结果**完全一致** —— 两条独立路径互证。
    #
    # ⚠️ `巴黎圣日尔曼` vs 词典原有的 `巴黎圣日**耳**曼` 只差一个字(尔/耳,同音异形)。
    #   这种最容易被眼睛跳过而以为「已经有了」。撞车检查:5 个中文名在
    #   TEAM_NAME_ZH 值集合与本表里都没被别的队占用(0 冲突)。
    # ── 巴甲 BRA_SERIE_A(2026-09-10 横幅点名「整个联赛在售场次全解不出:巴甲」)──
    # ⭐ 三条都是**同场唯一**锚出来的,不是按音写的。词典里这三支的英文键**本来就有**,
    #    只是竞彩用了另一种中文写法 ⇒ 属于「情况①:补 override」,不是「整队不在」。
    #
    #   · 科里蒂巴  ← 2026-09-12 在售那场:竞彩「科里蒂巴 vs 巴拉纳竞技」,而盘面
    #     当天 6 场巴甲里 `Atletico Paranaense`(=巴拉纳竞技,已解)客场**只出现一次**,
    #     对手正是 `Coritiba`。词典写「库里蒂巴」——「库/科」同音异形,最容易被眼睛跳过。
    #   · 沙佩科    ← 档案 79 场;与已解对手同场配对命中 3 次、候选**唯一**
    #     (2026-07-17 Bahia vs Chapecoense-sc · 2026-07-23 Chapecoense-sc vs Flamengo)。
    #     词典写「沙佩科恩斯」—— 竞彩用短写法。
    #   · 达伽马    ← 档案 38 场;同法命中 2 次、候选唯一
    #     (2026-07-16 Vitoria vs Vasco DA Gama · 2026-08-16 Vasco DA Gama vs Santos)。
    #     词典写「瓦斯科达伽马」—— 同样是短写法。
    #
    # ⛔ **同批还有 5 个巴甲写法锚不到,故意没补**(戈竞技 ×185 · 尤文图德 ×182 ·
    #    库亚巴 ×165 · 阿瓦伊 ×67 · 累体育 ×59):`jingcai_odds_history` 的英文列对它们
    #    **全是空**,盘面赛程只覆盖 2026-07-16→09-14 够不着它们的比赛,皇冠档案里
    #    **一场巴西联赛都没有**。⇒ 没有锚就不写 —— 错映射是静默污染,比缺映射更坏。
    #    ⚠️ 它们多半是往季降级队(盘面当前 20 支巴甲**全部**有中文),真回来了横幅会再点名。
    # ── 日乙 JPN_J2(2026-09-11 横幅「整个联赛的在售场次全部解不出:日乙」)──
    # ⭐ `仙台七夕` 是**第三种**变体机制,前两次都不是这个:
    #   ① 同音异形(弗洛/弗罗西诺内 · 库/科里蒂巴)· ② 长短写法(沙佩科/沙佩科恩斯)
    #   ③ **意译 vs 音译** —— 词典写「仙台维加塔」(Vegalta 音译),竞彩写「仙台七夕」
    #      (Vegalta = Vega+Altair,即牛郎织女 ⇒ 七夕)。**按音根本对不上**,
    #      只有 fixture 锚拿得到。⇒ 判类必须 grep **英文键**,grep 中文会判成「整队不在」。
    #
    # 锚(全部同场唯一):
    #   · 仙台七夕 ← **当前在售那场**:2026-09-13 日乙盘面只有 2 场,
    #     `Consadole Sapporo`(=札幌冈萨多,已解)客场只出现一次,对手正是 `Vegalta Sendai`。
    #     ⚠️ 档案对它只到 2025-11-29(涓流还没回填它的 2026),所以**只有在售那场能当锚**。
    #   · 秋田闪电 ← 2026-08-15 `Blaublitz Akita` vs `Kataller Toyama`(=富山胜利,已解),
    #     档案配对命中 2 次、候选唯一。词典写「秋田蓝色闪电」—— 又是长短写法。
    #
    # ⛔ **同批普查出日乙 14 个解不出,只补这 2 个**,另 12 个故意留着
    #    (熊本深红 ×404 · 群马温泉 ×391 · 爱媛FC ×332 · 磐城FC ×300〔09-25 已锚补〕· 山口雷诺 ×300 ·
    #     枥木SC ×290 · 金泽塞维 ×232 · 琉球FC ×221 · 盛冈仙鹤 ×182 · 藤枝MYFC ×162 ·
    #     今治FC ×86 · 相模原SC ×12):它们的档案比赛最晚停在 2021–2025,而盘面赛程只覆盖
    #     2026-08-07→09-13、AF fixture 缓存只到 2026-05-23 —— 两条锚源都够不着。
    #    ⇒ 没有锚就不写。哪天它们再上架,横幅会点名,那时当场就有锚。
    "仙台七夕": "Vegalta Sendai",            # 词典写作「仙台维加塔」(意译 vs 音译)
    "秋田闪电": "Blaublitz Akita",           # 词典写作「秋田蓝色闪电」
    # ── 2026-09-18 横幅「整个联赛的在售场次全部解不出:日乙」(1/51) ──
    # ⭐ **上面那段留的话应验了**:`藤枝MYFC ×162` 正是 09-11 故意没补的 12 个之一,
    #    当时写的是「哪天它们再上架,横幅会点名,那时当场就有锚」—— 今天就是那天。
    #    锚(第①档,当前在售那场):竞彩周六002 `2026-09-19T09:30Z`,
    #    **该时刻整个 J2 League 只有 1 条** fixture `1606658`
    #    `Fujieda MYFC` vs `Omiya Ardija`;再用**已解出的客队**(竞彩写
    #    `大宫松鼠RB`,主词典写「大宫松鼠」)收窄,仍是 1 条。
    # ⚠️ **必须走 `af_fixture` 锚而不是 odds_snapshots**:09-19 那场 Pinnacle 还没开,
    #    该表 0 行 —— 拿它当唯一锚源会把**能锚的场次误判成锚不到**。
    # ⭐ 二次确认来自**独立于 AF 的一条线**:盘面 `JPN_J2` 名单 20 支里
    #    有且仅有一个 `Fujieda MYFC`。
    # ⚠️ 这是长短写法(词典写「藤枝」、竞彩写「藤枝MYFC」),属情况① ⇒ 只补本表,
    #    **不动** team_name_zh。撞车:`藤枝MYFC` 在 TEAM_NAME_ZH 值集 / 本表 /
    #    `_ZH_TO_EN` 里 0 占用;档案 187 行的英文列**全 NULL**(没被别的队占过)。
    # 📌 剩下 **11 个仍然锚不到**:档案窗内日乙只有 8 场、涓流游标还在 2021-12-12,
    #    逐个试过配对锚**全部落空**(含藤枝自己 —— 它靠的是在售那场,不是档案)。
    "藤枝MYFC": "Fujieda MYFC",              # 词典写作「藤枝」—— 长短写法
    ...
    #
    # ── 2026-08-12 横幅「解不出 4/7 · 整联赛丢失:沙职 · 过半:解放者杯 2/4」──
    # 5 个解不出的名字里 4 个是 ① 类(队在词典、竞彩换了个更长的写法),
    # 第 5 个 `艾卜哈`(Abha)整队不在 → 进 team_name_zh.py,不在这里。
    #
    # ⭐ 锚 = **开球时刻 + 联赛 + 已知那一侧**,不是字符串相似度:
    #   利雅得青年 / 胡巴尔卡德西亚 ← 2026-08-13T18:00Z 沙职,AF 同刻唯一一场
    #                 `Al Shabab vs Al-Qadisiyah FC`(该日沙职只有 16:15/18:00 两场,
    #                 16:15 那场被 `艾卜哈 vs 拉斯决心` 占掉 ⇒ 排除法也指同一场)。
    #   基多体育大学  ← 2026-08-13T22:00Z 解放者杯 `Mirassol vs LDU de Quito`,
    #                 锚=米拉索尔(已解析)。主词典写「基多体大」。
    #   罗萨里奥中央  ← 2026-08-14T00:30Z 解放者杯 `Rosario Central vs Corinthians`,
    #                 锚=科林蒂安(已解析)。主词典写「罗萨里奥」。
    #
    # 🚨 `利雅得青年` 是这批里最危险的一个:按字符串相似度它**同时**像
    #   `利雅青年`(Al Shabab) 和 `利雅得`(Al Riyadh) —— 五五开,而那是两支不同的队。
    #   只有开球时刻锚能定死它。这正是「⛔ 绝不照译音猜」在实战里的样子。
    #
    # 撞车检查:4 个中文名在 TEAM_NAME_ZH 值集合与本表里都没被占用(0 冲突)。
    # ⚠️ 英文值取 AF fixture 缓存的拼法。`LDU de Quito` / `Rosario Central` 在
    #   odds_snapshots 各有 8 / 12 行佐证;`Al Shabab` / `Al-Qadisiyah FC` 是 0 行
    #   —— 沙职本就是本周刚接的新联赛,采集窗口比它短,**「0 行」在这里等于
    #   「还没抓过」而不是「不存在」**(同族:日乙 J2 那次因果反了的判断)。
    "利雅得青年": "Al Shabab",
    ...
    # 2026-08-19 —— 横幅点名 1/9(欧罗巴 贝尔格莱德红星 vs 比尔森)。① 类:队在系统里,
    # 竞彩用了更短的中文串。⭐ **两侧只有一侧解不出**:`贝尔格莱德红星`→`FK Crvena Zvezda`
    # 本来就通,拖住整场的是 `比尔森`(词典写`比尔森胜利`)——横幅点的是**比赛**不是队,
    # 别被它列的两个名字误导成两处都要补。
    #   锚 = 同场唯一:odds_snapshots `UEL 2026-08-20 FK Crvena Zvezda vs Plzen`
    #        (api_football,psc 1.72/4.16/4.18,ou_line 2.5)——已解析的那一侧当日 UEL
    #        只有这一个对手。英文值照抄盘面真在用的 `Plzen`,不是按音译写的。
    #   撞车检查:`比尔森` 在 TEAM_NAME_ZH 值集 / _ZH_OVERRIDES / _ZH_TO_EN 里 0 占用;
    #        全词典含该串的条目只有 `Plzen`→`比尔森胜利` 一条。
    #   ⭐ 顺带实测:竞彩**自己的档案**(外部来源)里 `比尔森` 出现 **25 场**
    #        (2022-07-21..2026-02-27),而 `比尔森胜利` **0 场** ⇒ 词典那个值从来没
    #        匹配过竞彩。这不是「竞彩改了写法」,是一个**存在四年的静默缺口** ——
    ...
    # 2026-08-20 横幅 1/54:[意甲] 弗洛西诺内 vs 尤文图斯。**又是只坏一侧**
    # (`尤文图斯`→`Juventus` 主词典本来就有)。
    # 🚨 **差点分类错**:按竞彩写法 grep「弗洛西」→ 0 命中,差点判成 ② 类(整队不在)。
    #    实际词典**有**这支队,写的是「弗**罗**西诺内」(罗),竞彩写「弗**洛**西诺内」(洛)
    #    —— **一个同音异形字**,同本表上方 `巴黎圣日尔曼` vs `圣日耳曼` 那个坑。
    #    ⇒ 判 ①/② 类**不能靠 grep 竞彩那个写法**,要 grep 英文键(`TEAM_NAME_ZH.get('Frosinone')`)。
    #   锚 = 同场唯一:odds_snapshots `ITA_SERIE_A 2026-08-23 Frosinone vs Juventus`
    #        (odds_api,psc 7.56/4.72/1.44,ou_line 2.75);另有 `COPPA_ITALIA 2026-08-16
    #        Frosinone vs Juve Stabia` 独立佐证。✅ 该队 2026 确在意甲(升级),数据说的不是我推的。
    #   撞车检查:`弗洛西诺内` 在 TEAM_NAME_ZH 值集 / _ZH_OVERRIDES / _ZH_TO_EN 里 0 占用。
    #   ⭐ 第三例「词典的中文值从没匹配过竞彩」:`弗罗西诺内` 竞彩档案 **0 场**
    #      (前两例:比尔森 25 vs 0、千叶市原 65 vs 0)。
    # 📌 档案侧另有短写法 `弗洛西诺`(30 场,2023-08..2024-05 上次意甲时期)——
    #    ⛔ **故意不补**:我们自有赛程只回溯到 2026-05-31、皇冠档案 2024-08 起,
    #    两者都够不着那段窗口 ⇒ 拿不到独立锚。同 `贝红星` 那条的处置。
    "弗洛西诺内": "Frosinone",
    # 📌 同批发现但**故意不补**(2026-08-19),写下来免得下次有人以为漏了:
    #    竞彩**历史档案**把红星写作 `贝红星`(21 场,全部在欧罗巴),而**实时源**写
    #    全称 `贝尔格莱德红星`(已能解)——正是本表上方记过的「同一支队两种长度,
    #    两边都得在表里」。⛔ 不补的唯一理由是**拿不到锚**:
    #      · league_predictions 只回溯到 2026-05-31、odds_snapshots 到 2026-06-10,
    #        而 `贝红星` 的场次全在 2026-02 及更早 ⇒ 我们自己的赛程数据够不着;
    #      · 皇冠档案同一场(163507,2026-02-26 贝红星 vs 里尔)的 `home_team` 也是
    #        NULL —— **同一个缺口出现在第二张表上,不是独立证据**;
    #      · 竞彩档案自己的英文侧同样 NULL。
    #    按红线「绝不照英文猜译名——错映射是静默污染,比缺映射更坏」,宁可留缺口。
    ...
    "布赖代合作": "Al Taawon",
    "穆拜赖兹征服": "Al-Fateh",
    # ── 亚运女足(2026-09-14 横幅)· **简称**那一半 ──────────────────
    # 全称已在 team_name_zh(`China W`/`Hong Kong W`,锚见那边的长注释)。
    # 竞彩这两场同时给两种写法,长度差很多:
    #     全称 `中国女足`   / 简称 `中国女`
    #     全称 `中国香港女足` / 简称 `中国港女`
    # `_canonical_from_any` 先试全称、够用了,但别的调用路径可能只拿得到简称
    # ——「只补一边,横幅会在另一条链上继续点名」那条教训的同一形状。
    # 撞车检查:两个简称在 TEAM_NAME_ZH 值集 / 本表 / _ZH_TO_EN 里 0 占用,
    # 也不是任何已有中文名的子串。
    "中国女": "China W",
    "中国港女": "Hong Kong W",
    # ── 亚冠精英 · 情况①「队在词典、竞彩换了种写法」(2026-09-14)─────────
    # 补完 36 条比分锚定后,档案 69 支里只剩 3 支解不出。这 2 支的**英文早在词典里**,
    # 只是竞彩用了另一种中文写法 ⇒ 属于情况①,补这里而不是 team_name_zh。
    # 锚:同一次比分锚定跑出来的(闸② 各 ×6 / ×2),不是按音写的。
    #   · 中央海岸(90 行)↔ 词典写「中央海岸水手」—— **长短写法**(竞彩用短的)
    #   · 费萨里(10 行)  ↔ 词典写「哈马赫费萨利」—— 同一支队,竞彩用短写法
    # ⛔ 第 3 支「波斯波利」(24 行)**故意不补** —— 闸③ 判它冲突
    ...
    # -- 2026-09-16 横幅 · 简称那一半 + 一条写法变体 ----------------------
    # 全称已在 team_name_zh(锚见那边的长注释)。竞彩这几场同时给两种长度:
    #     中国亚运男足/中国亚 · 朝鲜亚运男足/朝鲜亚 · 日本亚足/日本亚
    #     中国香港亚运男足/中国港亚 · 乌兹别克斯坦女足/乌兹别女
    # ⚠️ 简称短到「中国亚」「日本亚」这种 —— 正因为短才更要走锚:撞车检查过,
    #    5 个简称在 TEAM_NAME_ZH 值集 / 本表 / _ZH_TO_EN 里 0 占用。
    #
    # ⭐ `托林斯` 是**情况①**:英文键 `Torreense` 早在词典里,只是词典写「托雷恩塞」
    #    而竞彩写「托林斯」—— 同一支队两种中文写法(同 比尔森/千叶市原 那族)。
    #    锚:UEL 09-17T19:00Z,**主队已解出**(`Lillestrom`)⇒ 该时刻唯一
    #    fixture 1636287 `Lillestrom` vs `Torreense`。按音猜不出「托雷恩塞→托林斯」。
    "中国亚": "China PR U23",
    "朝鲜亚": "Korea DPR U23",
    "日本亚": "Japan U23",
    "中国港亚": "Hong Kong U23",
    ...
    "托林斯": "Torreense",
    # -- 2026-09-17 横幅 · 简称那一半 ------------------------------------
    # 全称 `沙特阿拉伯亚足` 已在 team_name_zh(锚见那边的长注释:fixture 1639466)。
    # 🚨 这条是「别猜简称」的实证:`沙特阿拉伯亚足` → `沙特亚`,**「阿拉伯」在
    #    中间被丢掉**。实测 7 支亚运队里简称是全称前缀的只有 5 支(另一个例外是
    #    `中国香港亚运男足`→`中国港亚`)⇒ 「取前缀」会静默造错 2/7。
    # ⚠️ 撞车检查:`沙特亚` 在 TEAM_NAME_ZH 值集 / 本表 / _ZH_TO_EN 里 0 占用;
    #    成年队 `沙特阿拉伯`→`Saudi Arabia` 是不同实体,未动。
    "沙特亚": "Saudi Arabia U23",
    # -- 2026-09-19 横幅 · 简称那一半 ------------------------------------
    # 全称在 team_name_zh(锚见那边:fixture 1639457 / 1639459)。
    # 🚨 `吉尔吉亚` 是「中间丢字」的**第三例**(丢「斯斯坦」),而 `伊朗亚` 是前缀 ——
    #    同一批里两种都有,正说明没有可推导的构词法。9 支里 3 支不是前缀。
    ...
    # 两种仍然都在,构词法依旧推不出来。
    "菲律宾女": "Philippines W",
    # -- 2026-09-21 横幅 · 英锦标赛的简称那一半 ---------------------------
    # 全称在 team_name_zh(锚见那边的长注释:**小组身份**,不碰队名)。
    # 六场里只有这 3 个简称与全称不同,另 3 个(克劳利/诺茨郡/布莱克浦)
    # 竞彩的 Abb 与 All **逐字相同**,不需要 override。
    ...
    "越南亚": "Vietnam U23",
    "韩国女": "South Korea W",
    # -- 2026-09-25 横幅 1/29 · 日乙 磐城FC vs 仙台七夕 ------------------------
    # 第 ① 类(队在词典里,竞彩换了写法):词典 `"Iwaki": "磐城"`,竞彩全称/简称都写「磐城FC」。
    # ⭐ 锚 = 开球时刻 + 已解出的对手:fixture 1606667 · 2026-09-26 08:00Z · J2 第 8 轮 ·
    #    `Iwaki`(主)vs `Vegalta Sendai`(客)。那一刻 AF 共 23 场(J2 11 场),按对手收窄唯一,
    #    主客方向与竞彩一致。AF 写法 `Iwaki` 与词典键逐字相同 ⇒ join 与卡片显示(「磐城」)都通。
    # ⚠️ 它是 09-11 那批「故意留着」的 12 个之一(见上面日乙段)—— 当时两条锚源都够不着,
    #    现在上架了,锚当场就有。同批的 爱媛FC / 琉球FC / 今治FC / 枥木SC / 相模原SC 看着是同一种
    #    长短写法,但**长得像是零证据** —— 仍然没锚,继续留着。
    "磐城FC": "Iwaki",
}
