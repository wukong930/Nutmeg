...
_LIBERTADORES_SAUDI_2026_08: dict[str, str] = {
    ...
    "Palestino": "巴勒斯人",
    # ── 沙职(2026-08-15 横幅)。**整队不在词典** ⇒ 放这里而不是 _ZH_OVERRIDES。
    # ⭐ 锚 = 开球时刻 + 已解出的另一侧:竞彩北京 2026-08-15 02:00 = UTC 08-14T18:00,
    #   AF ±90 分钟内 249 场,含 `Al-Hilal Saudi FC`(由竞彩**简称**`利雅新月` 解出,
    #   全称 `利雅得新月` 解不出 —— 正是本轮那个「两个字段都要试」的修救回来的)
    #   的**恰好 1 场**:`L307 Pro League 2026-08-14T18:00 Al-Hilal Saudi FC vs Al-Faisaly FC`。
    # ⚠️ 英文值取 `Al-Faisaly FC` 而不是 `Al-Faisaly KSA FC`:后者是 **closing 侧**
    #   (Odds API)的拼法,前者是 **gather 侧**(AF)的 —— 而竞彩 SP 要 join 的是
    #   `_gather_rows` 在用的那一侧。两者在库里从未互为对手(同行 0 次),
    #   是同一支队的跨源劈开,不是两支队。
    # 📌 那个跨源劈开本身(`Al-Hilal`/`Al-Hilal Saudi FC`、`Al-Faisaly KSA FC`/`Al-Faisaly FC`)
    #   是 ODDS_SOURCE_ALIASES 的候选,但同槽指纹共享数 = **0**(closing 逐次调价、
    #   predict_log 只有一个快照,三元组对不上)⇒ 按「宁缺勿错」**本轮不补**,另立。
    "Al-Faisaly FC": "哈马赫费萨利",
    # ── 解放者杯 · 2026-08-15 涓流第 3 条。⭐ 本批**证据最强**的一条:
    #   闸① 单独就够 —— 2024-06-05 ±1 天窗内 L13 比分 0:1 的候选**只有 1 场**
    #   `Huachipato 0:1 Gremio`;闸「已知另一侧」再独立确认一次(`格雷米奥`→Gremio)。
    #   两道互不依赖的判据指向同一答案。
    # ⚠️ 闸②「≥2 场」仍不满足(档案只有这 1 场)—— 同 [[San Lorenzo]] / [[Palestino]],
    #   本族三条都是这个形态,值得记一笔:**竞彩档案里的南美球队大多只出现过一两次**,
    #   把「≥2 场」当硬闸会让整个解放者杯永远补不完。判据要看**哪道闸在承重**。
    # ✅ 撞车:AF 名册含该词根的**只有 `Huachipato` 一个**,中文「瓦奇巴托」也没被占用。
    "Huachipato": "瓦奇巴托",
    ...
    # ── 第四轮(2026-08-12)。⚠️ 这批不是「又漏了」,是**涓流还在往前扫**:
    #    `days_remaining=1142`(游标 2023-06-25,终点 2026-08-10)⇒ 档案每扫一批
    #    就会冒出该时期的新队名。这是**正常的增量**,不是回归。
    #
    # ⭐ 关键的量:涓流 `StartInterval=3600`,**每小时推进 7 天** ⇒ 1142 天
    #    大约 **7 天跑完**(实测四轮:1170→1163→1156→1149→1142,每轮 −7)。
    #    所以 `test_jingcai_listed_teams_are_fully_reachable` 在这一周里会**反复变红**,
    #    那是**一次性回填的正常尾巴**,一周后自然停。
    #    ⛔ 不许因为「它老红」就去削弱它/加豁免名单 —— 我一开始把 1142 天错当成
    #    「3 年的跑步机」,差点据此改护栏口径;查了 StartInterval 才发现是一周。
    #    **先量节奏,再决定护栏该不该改。**
    #    锚一律 = 北京日 + 比分 + 已知对手(工具 `anchor_team_names_by_score.py`):
    "Racing Club": "竞技",              # 北京 2023-06-09 三场中唯一 2:1,主队 Flamengo 已知
    "Atletico Nacional": "麦国民",      # 同日唯一 3:0,主队 Olimpia 已知
    "Argentinos JRS": "阿青年人",       # 北京 2023-06-08 七场中唯一 2:1 —— 一场同钉两名
    "Liverpool Montevideo": "蒙利物浦",  # ↑ 同一场的客队
    # ⚠️ `天主青年` 这条**比其余弱一档,单独说明**:档案里它那场**没有比分**
    #    (match_id=1019607,11 行赔率但 home_goals/away_goals 全 NULL)⇒ 比分闸用不上。
    #    改用三条约束:① 2023 年 5 月整月的解放者杯只有 **2 场** Olimpia 赛事;
    #    ② 另一场(05-03 vs Atletico Nacional)已被 `麦国民` 占掉 ⇒ 排除法只剩一场;
    #    ③ 主客方向一致(Olimpia 在档案和 AF 里都是客队)。
    #    竞彩 close_date=05-25 而 AF 北京日=05-26 —— 在锚定工具本来就用的 ±1 窗内。
    ...
    "Al-Fateh": "穆拜征服",                     # ×11
    "Al Riyadh": "利雅得",                     # ×10
    # 2026-08-12 横幅「整联赛丢失:沙职」—— 第 19 支。上一批 18 支是从竞彩**历史
    # 档案**锚出来的,而 Abha 本赛季刚升上来/重回,档案里没有,所以那轮没覆盖到。
    # ⭐ 锚 = 开球时刻:2026-08-13T16:15Z 沙职,AF 同刻唯一一场 `Abha vs Al-Hazm`,
    #   而 `拉斯决心 → Al-Hazm` 已在册 ⇒ 对手侧把这场钉死。
...
#
# ⭐ 先逐名跑 `zh_to_canonical`:横幅点 1 场 = 2 个名字,而 `卡塔尔亚足` 09-15
#    就注册过了 ⇒ **真缺口只有主队那一侧**。横幅按**比赛**点名不是按队(第三次了)。
#    源码 1109 / daemon 1109、两边都是「解不出 1 场」⇒ 排除「改了没重启」
#    (体检第 10 类「词典未生效」)。
#
# 🚨 我第一版普查判据写错了:拿裸 `zh_to_canonical` 扫,扫出「解放者杯也全灭」。
#    生产判据是 `home_en and away_en`,而这两列由 `_canonical_from_any`
#    **全称→简称两个都试**填 —— `德尔瓦耶独立` 全称解不出、简称解得出,不是缺口。
#    换回生产口径后 1/23,与 daemon 一致。⇒ 复用函数 ≠ 复用口径。
#
# ## 锚:第①档(当前在售那场本身),唯一性验过两层
#
#   竞彩 周五001 09-18T10:30Z `沙特阿拉伯亚足` vs `卡塔尔亚足`
#     → 本地 AF 缓存里 `Asian Games` 该时刻**唯一** fixture 1639466
#       `Saudi Arabia U23` vs `Qatar U23`;再用**已解出的客队** `Qatar U23`
#       二次收窄,仍是 1 条。主客方向与竞彩一致。
#
# ## 为什么只补这 1 支,不把 AF 那 15 支 U23 一次补完
#
# AF 赛程里这项赛事共 **15 支 U23**,词典里只有 7 支。但竞彩的中文串**没有可推导
# 的构词法**。实测 7 支已注册亚运队:简称是全称前缀的只有 **5 支**,
# `中国香港亚运男足`→`中国港亚`、`沙特阿拉伯亚足`→`沙特亚` 都在**中间**丢字
# (「香」、「阿拉伯」)⇒ 「取前缀」这条规则会静默造错 2/7;后缀本身还分
# 「亚足」与「亚运男足」两种写法。照英文猜译名 / 猜简称 = 静默污染,比缺映射更坏。
# 未映射历史(`logs/sporttery_unmapped_history.jsonl`,670 轮 forward-only)里
# 出现过的「亚运」中文串**总共只有 6 个**,除本条外全部已注册 ⇒ 另外 8 支竞彩
# **从未上架过**,一个可用的锚都没有。⇒ 能锚的补、锚不到的留着等它上架
# (第①档锚的性质就是「横幅点名之时正是它最可用之时」)。
#
# ⚠️ 撞车检查:`Saudi Arabia U23` 不在 TEAM_NAME_ZH;`沙特阿拉伯亚足` / `沙特亚`
#    在 TEAM_NAME_ZH 值集 / `_ZH_OVERRIDES` / `_ZH_TO_EN` 三个命名空间里**均 0 占用**。
#    成年队 `Saudi Arabia`→`沙特阿拉伯` 是**不同实体**,未动。
_BANNER_2026_09_17: dict[str, str] = {
    "Saudi Arabia U23": "沙特阿拉伯亚足",
}
TEAM_NAME_ZH.update(_BANNER_2026_09_17)


# -- 2026-09-19 横幅(2/60)· 亚运男足 伊朗 + 吉尔吉斯斯坦 -----------------------
#
# ⭐ **第三次兑现同一条策略**:09-15 那批 AF 有 15 支 U23、我只补了当时能锚的,
#    并写下「剩下的竞彩从未上架过、一个锚都没有 ⇒ 留着等它上架」。
#    09-17 沙特上架补了 1 支,今天伊朗 + 吉尔吉斯斯坦上架,又各自当场拿到第①档锚。
#    ⇒ 「等」在这里是**合法计划**:横幅会主动点名,且点名之时证据最全。
#
# ## 锚:两条都是第①档,该刻该赛事**本来就唯一**(收窄前后都是 1 条)
#
#   竞彩 周日001 09-20T05:00Z 伊朗亚运男足 vs 中国亚运男足
#     → fixture 1639457 `Iran U23` vs `China PR U23`(客队已解出)
#   竞彩 周日005 09-20T10:30Z 吉尔吉斯斯坦亚足 vs 日本亚足
#     → fixture 1639459 `Kyrgyz Republic U23` vs `Japan U23`(客队已解出)
#
# ## 🚨 简称「中间丢字」再添一例 —— 前缀规则的错误率从 2/7 涨到 3/9
#
#     伊朗亚运男足    → 伊朗亚     前缀 ✅
#     吉尔吉斯斯坦亚足 → 吉尔吉亚   **丢掉「斯斯坦」** ❌
#
# 连同已有的 `中国香港亚运男足→中国港亚`、`沙特阿拉伯亚足→沙特亚`,
# 9 支里 3 支不是前缀 ⇒ 按全称推简称会静默造错 1/3。⛔ 简称必须走锚。
#
# ## 🚨 顺带发现旗帜护栏的一个盲区(已在测试里堵上)
#
# `Kyrgyz Republic U23` 的底名是 `Kyrgyz Republic`,而旗帜表里只有 `Kyrgyzstan`
# —— 那条「自发现」护栏的判据是 `if 底名 in 旗帜表`,**底名不在表里就整条跳过**,
# 于是它既不要求补旗、也不会红,卡片静默退回字母缩写「Ky」。
# ⇒ 护栏的**发现判据本身**不在被检查之列。已补第二条判据(见测试)。
#
# ⚠️ 撞车检查:4 个中文串在 TEAM_NAME_ZH 值集 / `_ZH_OVERRIDES` / `_ZH_TO_EN` 里
#    **均 0 占用**;`Iran U23` / `Kyrgyz Republic U23` 两个英文键均不在词典。
#    成年队 `Iran`→`伊朗` 未动;`Kyrgyz Republic` 成年队**词典里本来就没有**,也没造。
_BANNER_2026_09_19: dict[str, str] = {
    "Iran U23": "伊朗亚运男足",
    "Kyrgyz Republic U23": "吉尔吉斯斯坦亚足",
}
TEAM_NAME_ZH.update(_BANNER_2026_09_19)


# -- 2026-09-20 横幅(1/31)· 亚运女足 菲律宾 ------------------------------------
#
# ⭐ 第四次兑现同一条策略(09-17 沙特 · 09-19 伊朗+吉尔吉斯 · 今天菲律宾)。
#
# ## 🚨 这场的锚**该时刻真的不唯一** —— 前几次都是「本来就唯一」,这次不是
#
#   竞彩 周一001 09-21T07:00Z 中国女足 vs 菲律宾女足
#     → `Asian Games Women` 该时刻有 **4 条** fixture:
#         Bangladesh W vs Myanmar W · China W vs Philippines W ·
#         South Korea W vs Korea DPR W · Uzbekistan W vs Hong Kong W
#     → 用**已解出的主队** `China W` 收窄 ⇒ **1 条** fixture 1639558,客队 `Philippines W`
#   ⇒ 「同场已解出的另一侧」这一档在这里是**承重的**,不是装饰。
#
# ## 🚨 顺带发现:我 09-19 才补的那条旗帜护栏,**今天就被隔壁那一半绕过去了**
#
# 昨天的判据是「中文值以 `亚足`/`亚运男足` 收尾 ⇒ 按构造全是国家队」。
# 这句话对**男**足成立,对**女**足**不成立** —— NWSL 的俱乐部女队中文名也以
# `女足` 收尾(天使城女足 / 波特兰荆棘女足 …共 8 支)。照着放宽会要求给
# **俱乐部**发国旗,正是那条设计当初明确不肯做的事。
# ⇒ 判据换成一个**外部可枚举**的人口:**词典条目 ∩ AF 亚运赛程出场队**
#   (亚运足球只有国家队参赛)。男女通吃、俱乐部自动排除、且不依赖旗帜表本身。
#
# ⚠️ 撞车检查:`菲律宾女足` / `菲律宾女` 在 TEAM_NAME_ZH 值集 / `_ZH_OVERRIDES` /
#    `_ZH_TO_EN` 里**均 0 占用**;`Philippines W` 不在词典。
#    ⚠️ 成年队 `Philippines` 词典里**本来就没有**,没顺手造;
#       男足那支 `Philippines U23` 也**不在**词典(竞彩没上架过)—— 别混为一谈。
_BANNER_2026_09_20: dict[str, str] = {
    "Philippines W": "菲律宾女足",
}
TEAM_NAME_ZH.update(_BANNER_2026_09_20)


# -- 2026-09-21 横幅(3/4)· 英锦标赛(EFL Trophy)· 整联赛 6 支全新 ------------
#
# 🚨 **这批和前四批都不一样:六个名字全不在词典,而且前四档锚全部够不着。**
#   · 第①档(在售那场):该时刻该赛事 AF 有 **3** 场、竞彩也有 3 场,
#     而**六个名字一个都解不出** ⇒ 没有「已解出的另一侧」可以收窄,3×3 歧义。
#   · 第②③档(档案):四个名字档案 0 行;`布莱克浦`/`克劳利` 有行,
#     但对家全是英甲/英锦标赛的低级别队、**也解不出**(实测英甲几乎整片空白)。
#   · 多日期交集法:**对照组当场证伪** —— 「每个档案日期都有比赛」这个约束
#     对**曼城**都不成立(AF 缓存是按联赛/日期部分抓的)。0/6 通过。
#
# ## ⭐ 锚是 **小组身份 + 「竞彩不上 U21」**,两步,完全不碰队名
#
# 把竞彩原始记录**整条 dump** 出来才看见两个从没用过的字段:
# `homeRank`/`awayRank` 里写着小组。对上 AF fixture 的 `league.round`:
#
#     竞彩 周二002  [Southern Group H4] / [H3]   ↔  AF `Group South - 8`
#     竞彩 周二003  [Northern Group F4] / [F2]   ↔  AF `Group North - 6`
#     竞彩 周二004  [Northern Group B1] / [B2]   ↔  AF `Group North - 2`
#
#   字母↔组号:南北先把 002 独立钉死(该时刻只有一场 `Group South`)⇒ 得到
#   **H↔8** ⇒「字母=位次」被确认 ⇒ 套到另两行 F→6 / B→2 与 AF 逐字吻合。
#   结构佐证:AF 的 round 恰好是 `Group North/South 1..8`、每组 4 队。
#
# 🚨 **但小组身份只把 3 收窄到 2,没闭合** —— 这是护栏当场抓出来的,不是我看出来的:
#    小组末轮两场**同时开**,每组 2 场。我最初只在「含那 6 个关键词」的子集里数,
#    看到 3 场就以为唯一了。⇒ 又一次「我数的是我以为的人口」。
#    ⚠️ 也试过用竞彩给的名次(B1/B2)反推积分闭合 —— **不行**:这 3 个组共 6 场
#       缓存里只有 1 场有比分,积分算不出来。
#
# ## ⇒ 闭合靠一条**量出来的**经验规律,不是假设
#
# 竞彩英锦标赛档案 **88 场 / 56 个中文名(2021-09 → 2025-12)**,
# **一支 U21 学院队都没上过**(0 个名字含 U21/青年/二队/预备)。
# 而 EFL Trophy 每组是 3 支成年队 + 1 支受邀 U21,该时刻每组的 2 场里
# **恰好一场含 U21** ⇒ 竞彩那场必是另一场,唯一。三组逐一验过。
#
# ⛔ 没有用 `homeTeamAbbEnName`(MIK/CWT/NOS/GRI/WIG/BLP)。那是竞彩自己的英文
#    缩写,拿它去匹配全名仍然是「长得像」,零证据。小组身份不依赖任何名字。
#
# ⚠️ 英文键**照抄盘面在用的拼法**(`odds_snapshots`),逐个查过**各自唯一**、
#    无竞争写法(例:盘面写 `Grimsby` 不是 `Grimsby Town`、`Wigan` 不是
#    `Wigan Athletic`)。六支目前都只在 `EFL_CUP` 下出现过。
# ⚠️ 撞车:6 个英文键均不在 TEAM_NAME_ZH;6 个中文串在值集 / `_ZH_OVERRIDES` /
#    `_ZH_TO_EN` 里 0 占用。
# ⚠️ 这六支是**俱乐部**,不发国旗(`_NATION_FLAG` 未动)。
_BANNER_2026_09_21: dict[str, str] = {
...
TEAM_NAME_ZH.update(_BANNER_2026_09_21)


# -- 2026-09-21b · 斯托克港(英锦标赛队名普查的唯一收获)-------------------------
#
# ⭐ 背景:英锦标赛 47 支成年队里 37 支缺中文名,竞彩档案有 40 个未解出的中文串。
#    拿「档案配对 + 迭代到不动点」跑完整人口(英锦/英甲/英乙/足总杯/联赛杯/英冠
#    共 1680 场),**第 2 轮就收敛,只锚到这 1 个**。其余 39 个留着。
#
# 锚(第③档,档案配对):
#   档案 2026-05-24 [英甲] `博尔顿 vs 斯托克港`,主队已解出 `Bolton`
#     → 该日 `League One` 里 `Bolton` 主场的对手**唯一** = `Stockport County`
#
# ⚠️ **只有 1 场证据** —— 它的 14 场档案里,对家能解出的有 6 场,但另 5 场的日期
#    落在 AF/盘面覆盖之外。本仓有「同场锚 1 次 + 候选唯一」的先例
#    (`COPA_LIBERTADORES` 的 `Estudiantes La Plata` 等),且候选集**无第二个**。
# ⚠️ 两道独立检查:① 盘面含 Stockport 的拼法**只有** `Stockport County`
#    (照抄 join 目标,不是按音写);② `斯托克港` 在 TEAM_NAME_ZH 值集 /
#    `_ZH_OVERRIDES` / `_ZH_TO_EN` 里 0 占用。
# 🚨 顺带记一个我做错的诊断:我先报「63% 档案行落在证据窗内」,那个指标量的是
#    **「这个日期字符串在某处出现过」**,不是「那天有这个联赛的 fixture」——
#    假安慰。逐行查才看清:英冠「一侧已解出」的 249 行里 **245 行找不到对应
#    fixture**(命中的 4 行还全在日期偏移 **−1**,竞彩是北京日期)。
_BANNER_2026_09_21B: dict[str, str] = {
    "Stockport County": "斯托克港",
}
TEAM_NAME_ZH.update(_BANNER_2026_09_21B)


# -- 2026-09-22 横幅(2/6)· 亚运男足 阿联酋 + 泰国 ------------------------------
#
# ⭐ **第五次**兑现「锚不到就留着」。横幅点 2 场,两场的**主队都已注册**
#    ⇒ 真缺口只有客队两侧。
#
# ## 锚:第①档,该刻**不唯一**,靠已解出的主队收窄
#
#   竞彩 周三001 09-23T05:30Z 中国亚运男足 vs 阿联酋亚运男足
#     → `Asian Games` 该刻 **2** 场(另一场 `Iran U23 vs Korea DPR U23`)
#     → 用已解出的主队 `China PR U23` 收窄 ⇒ fixture 1639462,客队 `UAE U23`
#   竞彩 周三002 09-23T10:30Z 日本亚足 vs 泰国亚运男足
#     → 该刻 **2** 场(另一场 `Kyrgyz Republic U23 vs Hong Kong U23`)
#     → 用 `Japan U23` 收窄 ⇒ fixture 1639463,客队 `Thailand U23`
#
# ## 🚨 小组佐证在这里**不成立** —— 差点按昨天的类比误用
#
# 竞彩给了 `[Group B2]` / `[Group A1]`,而昨天英锦标赛正是靠「竞彩小组 ↔ AF
# `league.round`」锚死的。但 AF 对亚运写的是 **`Group Stage - 3`**(第 3 **轮**),
# 不是组别 —— **同一个字段名,不同赛事语义完全不同**。
# ⇒ 它在这里是**零信息**,没拿它当证据。判据只有「该刻 + 已解出的主队」。
#
# ⚠️ 撞车检查:4 个中文串在 TEAM_NAME_ZH 值集 / `_ZH_OVERRIDES` / `_ZH_TO_EN`
#    里**均 0 占用**;`UAE U23` / `Thailand U23` 两个英文键不在词典。
#    成年队 `UAE`→`阿联酋`、`Thailand`→`泰国` 是**不同实体**,均未动。
# ⚠️ 两个简称**都是前缀**(阿联酋亚运男足→阿联酋亚 · 泰国亚运男足→泰国亚)——
#    与「前缀规则会造错 5/13」不矛盾:正因为两种都有,才推不出构词法。
_BANNER_2026_09_22: dict[str, str] = {
    "UAE U23": "阿联酋亚运男足",
    "Thailand U23": "泰国亚运男足",
}
...
def coverage_by_league() -> Dict[str, int]:
    ...
    return {
        ...
        # 2026-09-17 — 横幅 1/23:亚运男足 沙特(第①档赛事锚).
        "BANNER_2026_09_17":       len(_BANNER_2026_09_17),
        # 2026-09-19 — 横幅 2/60:亚运男足 伊朗 + 吉尔吉斯斯坦(第①档赛事锚).
        "BANNER_2026_09_19":       len(_BANNER_2026_09_19),
        # 2026-09-20 — 横幅 1/31:亚运女足 菲律宾(第①档锚,靠已解出的主队收窄).
        "BANNER_2026_09_20":       len(_BANNER_2026_09_20),
        # 2026-09-21 — 横幅 3/4:英锦标赛 6 支(**小组身份锚**,前四档全够不着).
        "BANNER_2026_09_21":       len(_BANNER_2026_09_21),
        # 2026-09-21b — 英锦标赛队名普查的唯一收获(档案配对锚,1 场证据).
