...
#
# ⚠️ **第二个已知错误(2026-08-03):σ_P 被当成与 P 无关,但它不是。**
# 本常数(以及 sigma_p_at / FREEZE_SIGMA_COEF)都假设 σ_P 只随 h 和腿身份变;
# 门槛公式再拿它乘 SP≈1/P,于是冷腿被 1/P 重罚。探索性测量(453 条轨迹,
# 复用 sigma_p_fit 口径,按锚点 P 分档)说这个假设不成立:
#     P 中位 35.6% → σ_P 1.62pp     P 中位 21.8% → σ_P 1.01pp
#     P 中位 10.8% → σ_P 0.80pp     P 中位 83.8% → σ_P 1.19pp
# σ_P 随 P 近乎成比例缩(0.046 / 0.046),**正好把 1/P 的放大抵消掉** ⇒
# 换算成 σ_EV,现行公式对甜区偏低 0.73×、对冷门过陡 1.52×。
# ⚠️ 那是**探索性**结果(问出来的,不是预注册出来的),而且 σ_EV 一侧用公允
# SP=1/P 当代理、混进了竞彩抽水的腿位结构 —— **σ_P 干净,σ_EV 不干净**。
# 修法 = docs/sigma_p_pdependence_prereg_v2.2_2026-08-03.md(**已生效 2026-08-04**),
# 挂在 v2.1 的同一个评估点上。⛔ **生效 ≠ 可以改这个数** —— 生效锁的是判据,
# 常数要等 §3 评估点(13 训练联赛累计 **300 条轨迹**)且 §4 全过才动。到点前别碰。
# 方向已定 §5.1=A(两侧都按实测修),但冷门段另有 §4⓪′ 样本量闸(轨迹数 <150 退回)。
SIGMA_P = 0.012
