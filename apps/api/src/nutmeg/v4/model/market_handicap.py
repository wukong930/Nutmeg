...
# ⭐ v1.9(2026-07-28):随机效应收缩后的点估。旧值 v1.7 为 0.046 / 0.016。
# 收缩把两条线各自的 δ̂(0.0456 / 0.0241)向随机效应总体 0.03667 拉:
# 精度高的 −1 少收缩(0.0456→0.0411)、精度低的 +1 多收缩(0.0241→0.0322)。
# 详细理由 + 26% 功效的诚实边界见上方 SE 那段与 prereg v1.9。
_C1_DELTA = 0.04113     # −1 线:让胜 → 让平(v1.7: 0.046)
_C1_DELTA_P1 = 0.03220  # +1 线:让负 → 让平(镜像)(v1.7: 0.016)
...
_C1_DELTA_SE = 0.00693     # 收缩后 SE(δ₋₁);v1.7 为 0.010(N=1,882 朴素二项)
_C1_DELTA_P1_SE = 0.00787  # 收缩后 SE(δ₊₁);v1.7 为 0.0135
