...
#
# ── 换盘史 ──────────────────────────────────────────────────────────
# 2026-07-15T07:00:00  cutoff 2024-08-01(冻结 724 天)→ cutoff 2026-06-01
# 2026-09-15T08:35:00  cutoff 2026-06-01 → **cutoff 2026-09-15**(本次)
#   起因不是 booster:football-data 自 2026-01-14 起不发 Pinnacle ⇒ 训练行几乎没长
#   (29,347 → 29,336,还少 11 行:验证窗 227→340 天把它们挪进了 val)。
#   买到的是 **team_state 快照**:445 → 466 对(+21 支新队),两边都有的 445 对里
#   **231 对状态变了**,而那 231 对**恰好等于**「cutoff 后参赛过 ∩ 旧快照里有」
#   ——「变了但没参赛」「参赛了但没变」都是 0 ⇒ 改动完全由真实比赛驱动。
#   ⚠️ `--validation-days 340` 是必需的:默认 90 会让验证窗整个落进无 Pinnacle 区
#      ⇒ val=0 ⇒ 温度校准硬失败(train.py 那道闸)。这个数会随 cutoff 前移继续涨。
#
# ⚠️ 更新它的**直接后果**:Layer A 的可拟合人口**当场清零**(旧代 637 条被排除),
#    要按 8 周回看 / 最少 30 条重新攒。实测节奏约 2-3 天过门槛、一周内恢复。
#    这不是副作用,**这就是它的目的** —— 旧盘的偏差不该被拟合成新盘的修正。
CURRENT_ARTIFACT_ERA_START = "2026-09-15T08:35:00"
