...
def test_probability_preserving_admission_passes_when_active_folds_pass() -> None:
    ...
    assert report.status == "shadow_admission_passed"
    ...
    assert "replacement_probability_preserving_admission:small_changed_sample" in (
        report.warnings
    )


def test_probability_preserving_admission_watchlists_when_fold_coverage_is_missing() -> None:
    audit_report = _audit_report()
    competition_gate_report = _competition_gate_report()
    grid_report = _grid_report(audit_report, competition_gate_report)
    ...
    assert report.status == "shadow_admission_watchlist"
    assert report.shadow_candidate_allowed is False
    assert "active_season_fold_count" in failed_checks


def test_probability_preserving_admission_cli_options_and_main(tmp_path: Path) -> None:
    audit_report = _audit_report()
    competition_gate_report = _competition_gate_report()
    grid_report = _grid_report(audit_report, competition_gate_report)
    ...
    args = _parse_args(
        [
            ...
            "0.1",
            "--min-overall-average-hit-probability-delta-vs-original=-0.02",
            ...
            "0.0",
            "--min-fold-average-hit-probability-delta-vs-original=-0.02",
    ...
    assert options.min_overall_final_answer_count == 2
    assert options.min_overall_profit_loss_delta_vs_original == 0.1
    assert options.min_fold_average_hit_probability_delta_vs_original == -0.02
    assert options.min_active_season_fold_count == 2
    assert options.require_no_production_change is False
    assert options.max_report_folds == 20
    ...
    saved = load_historical_replacement_probability_preserving_admission_report(
        output_path
    )
    assert saved.status == "shadow_admission_passed"
...
def _audit_report() -> HistoricalCandidateMarginalAuditReport:
    ...
    return HistoricalCandidateMarginalAuditReport(
        report_key="historical_candidate_marginal_audit:probability_preserving_test",
        status="generated",
        slice_count=2,
        competition_count=1,
...
def _competition_gate_report() -> HistoricalShortOddsCompetitionGateReport:
    ...
    return HistoricalShortOddsCompetitionGateReport(
        report_key="historical_short_odds_competition_gate:probability_preserving_test",
        status="generated",
...
def _item(
    ...
    *,
    ...
    model_top = _replacement(
        ...
        simulated_profit=1.0 if original_hit else -2.0,
    ...
    replacement = _replacement(
        ...
        decimal_odds=1.95,
        model_edge=0.08,
        profit_loss_delta=replacement_profit - (1.0 if original_hit else -2.0),
    )
    return HistoricalCandidateMarginalAuditItem(
        ...
        original_profit_loss=1.0 if original_hit else -2.0,
