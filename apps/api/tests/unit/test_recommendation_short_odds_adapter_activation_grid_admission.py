...
def test_activation_grid_admission_accepts_candidate_across_folds() -> None:
    audit_report = _audit_report(
        [
            _audit_item(suffix="a", competition_id="FRA_LIGUE_2"),
            _audit_item(suffix="b", competition_id="FRA_LIGUE_2"),
        ]
    )
    ...
    report = build_short_odds_adapter_activation_grid_admission_report(
        ...
        options=_admission_options(min_active_competition_fold_count=1),
    )

    assert report.status == "accepted_candidate_found"
    assert report.production_candidate_allowed is True
    assert report.accepted_candidate_count == 1
    assert report.best_candidate is not None
    assert report.best_candidate.status == "accepted"
    assert report.best_candidate.rolling_active_competition_fold_count == 1
    assert report.best_candidate.rolling_failed_fold_count == 0
    assert report.best_candidate.rolling_failed_checks == []


def test_activation_grid_admission_keeps_candidate_shadow_only_when_fold_coverage_is_low() -> None:
    audit_report = _audit_report(
        [
            _audit_item(suffix="a", competition_id="FRA_LIGUE_2"),
            _audit_item(suffix="b", competition_id="FRA_LIGUE_2"),
    ...
    assert report.status == "shadow_only_candidates"
    assert report.production_candidate_allowed is False
    assert report.shadow_allowed is True
    assert report.shadow_only_candidate_count == 1
    assert report.best_candidate is not None
    assert report.best_candidate.status == "shadow_only"
    assert "active_competition_fold_count" in report.best_candidate.rolling_failed_checks


def test_activation_grid_admission_cli_writes_report(tmp_path: Path) -> None:
    audit_report = _audit_report(
        [
            _audit_item(suffix="a", competition_id="FRA_LIGUE_2"),
            _audit_item(suffix="b", competition_id="FRA_LIGUE_2"),
    ...
    saved = load_short_odds_adapter_activation_grid_admission_report(output_path)
