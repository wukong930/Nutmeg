...
| W | Theme | Deliverable | Estimate |
|---|---|---|---|
| **V6 W5** ⚠️ | Lineup feature ablation | 380×2 EPL fixtures ingested (lineups + injuries); ablation found 7 of 9 W2 features fail; 1 features (30-day unique injured) saves −0.0038 log-loss on EPL 1-fold under strict leak controls. Multi-season + multi-league validation deferred (out of W5 budget). See [v6_w5_lineup_ablation.md](archive/v6/v6_w5_lineup_ablation.md) | shipped |
| **V6 W6** ✅ | Multi-fold lineup validation | La Liga 23/24 + 24/25 ingest; 4-cutoff × 2-league ablation; recent_n_injuries beats baseline 3/4 folds with mean −0.0020 log-loss. Wired into `feature_columns_with_lineups()`. See [v6_w6_lineup_validation.md](archive/v6/v6_w6_lineup_validation.md) | shipped |
| **V6 W7** ✅ | Lineup-aware artifact (opt-in) | `nutmeg-train --with-lineups` trains with V6 W6 validated features; metadata flags artifact; `build_features_for_fixtures` auto-runs lineup transform; A/B demo shows expected λ shifts. Default stays lineup-free until W8 live ROI confirms. | shipped |
| **V6 W8** ✅ | Live observation + A/B cron | `nutmeg-refresh-lineups` daily cache pull; GH Actions heartbeat; `nutmeg.v4.observation.ab_report` + `nutmeg-ab-report` CLI slices settlements by `metadata.with_lineups`; 15 new tests (393/393 V4 suite passing). 4-week real-bet ROI decision deferred until data accumulates. See [v6_w8_observation_onboarding.md](archive/v6/v6_w8_observation_onboarding.md) | shipped |

### P2 — Product polish + handoff (W9-12)

| W | Theme | Deliverable | Estimate |
|---|---|---|---|
| **V6 W9** ✅ | User-flow product CLI | `nutmeg-rec` 交互入口: 类型菜单 → 比赛 CSV → 资金 → 推荐. 新增 `combo.single_match` 单关 recommender (top_per_match=1 防止同场多投); 串关 + 复式 复用 V5/V6 引擎; Chinese-localized prompts + output; 全 ¥2/¥20k/31.5% vig 规则. 25 new tests (418/418 V4 suite passing). See [v6_w9_user_flow.md](archive/v6/v6_w9_user_flow.md) | shipped |
| **V6 W10** ✅ | Chinese dashboard + 规则说明 | 新增 `GET /api/v4/rules` 端点暴露 `JINGCAI_DEFAULT` JSON; dashboard 加 `⑤ 规则说明` tab (投注规则 / 派奖机制 / 推荐门槛 / 玩法说明 / 风险提示) + 顶部 `rules-hint` 速查条; 修复 "of bankroll" 残余英文; 投注/凯利 input 步长锁 ¥2. 10 new tests (428/428 V4 suite passing). See [v6_w10_chinese_dashboard.md](archive/v6/v6_w10_chinese_dashboard.md) | shipped |
| **V6 W11** ✅ | 杯赛 + 国家队基础 | `competitions.py` 注册表 (UCL/UEL/UECL/FAC/COPA_DEL_REY/COPPA_ITALIA/DFB_POKAL/COUPE/WC/EURO/COPA_AMERICA/WC_QUAL_UEFA, API-Football ID 已合入 `API_FOOTBALL_LEAGUE_IDS`); `cup_features.py` 5 列 side-channel (is_cup_match/is_knockout/is_two_legged/is_national_team_match/competition_type_id); `lookup_cup_team_pair` 跨联赛 team_state 查找. 40 new tests (468/468 V4 suite passing). 杯赛训练数据回填留 V7. See [v6_w11_cup_data.md](archive/v6/v6_w11_cup_data.md) | shipped |
| **V6 W12** ✅ | V6_HANDOFF + ship | [V6_HANDOFF.md](V6_HANDOFF.md) (single source of truth), [v6_w12_retrospective.md](v6_w12_retrospective.md) (what worked / what didn't / V7 backlog); tagged `v6.0-shipped`. 468/468 V4 suite passing. | shipped |
