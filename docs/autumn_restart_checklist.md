...
- [ ] 🏆 **AF 采集路带上新注册的杯赛**(2026-08-16 记,**有验证窗口再做**)——
  社区盾暴露的单点。⭐ 症状是「竞彩上架、盘面无 Pinnacle ⇒ 该场静默不可投」。
  · **Odds API 结构性没有这个赛事**:`/sports?all=true` 175 个 sport 里含 "shield" 的 **0** 个
    ⇒ 补 `SPORT_KEYS` 这条路走不通,别去试。
  · **AF 有**:`/odds?league=528&season=2026` 实调返回 13 家博彩,含
    **Pinnacle 2.78/3.38/2.65**(fixture 1582365)。但我们**从没问过 528** ——
    `_odds` 缓存 19,157 个文件里 league 528 **零行**,因为它 2026-08-16 才注册。
  · 已做:`competitions.py` + `league_labels.py` 注册完成(commit `eccccc8`),
    `API_FOOTBALL_LEAGUE_IDS` 自动合并 ⇒ 528 已在。**缺的是采集 cron 的联赛集**。
  · 🚨 **别急**:量过了,窟窿是 **1 场不是一片** —— 今天在售 27 场里 26 场有 Pinnacle;
    近 60 天竞彩上架的 9 个联赛**零个**完全无覆盖。
    ⚠️ 但那个「近 60 天」有盲区:档案只到 2026-08-01(涓流游标还在 2025-05),
    **不含 8 月中旬** ⇒ 社区盾这类是不是新出现的一族,要等涓流跟上才知道。
  · 验证方式:补完后下一个同类赛事(欧超杯/西超杯/各国超级杯)上架时,
    盘面应出现该 league 的 psc_* 行。⛔ 别拿社区盾验 —— 它一年一场。

  ⭐ **这条的真正价值不是那一场比赛,是它暴露的形状**:
  「竞彩上架 + 队名解得出 + 注册表有 ⇒ 仍然不可投」,因为**赔率是第四条独立的链**,
  而横幅/体检都看不见它([[unmapped-banner-silences-not-fixes]] 的杯赛版)。
  ⚠️ 社区盾那场三条腿 EV 全负(最好的 −4.80%)⇒ 系统不显示它**结论碰巧对、理由是错的**。
  换一场真 +EV 的杯赛,同样的断链会让它**静默看不见**。
