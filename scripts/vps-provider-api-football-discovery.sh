...
TARGET_COMPETITION_NAME="${NUTMEG_API_FOOTBALL_DISCOVERY_COMPETITION:-Premier League}"
TARGET_COUNTRY_NAME="${NUTMEG_API_FOOTBALL_DISCOVERY_COUNTRY:-England}"
TARGET_SEASON="${NUTMEG_API_FOOTBALL_DISCOVERY_SEASON:-2025}"
MIN_COMPETITION_SCORE="${NUTMEG_API_FOOTBALL_DISCOVERY_MIN_COMPETITION_SCORE:-0.75}"
MAX_COMPETITION_CANDIDATES="${NUTMEG_API_FOOTBALL_DISCOVERY_MAX_COMPETITIONS:-5}"
MAX_SEASON_CANDIDATES="${NUTMEG_API_FOOTBALL_DISCOVERY_MAX_SEASONS:-6}"
...
python3 - "$ADMIN_TOKEN" "$DISCOVERY_PAYLOAD_B64" <<'PY'
from __future__ import annotations
...
league_id = recommended_competition.get("provider_competition_id") or "none"
season_id = recommended_season.get("provider_season_id") or "none"

print(
    "api_football_competition_discovery_ok "
    f"target={result['target_competition_name']!r} "
    f"country={result['target_country_name']!r} "
    f"season={result['target_season']!r} "
    f"min_score={result['min_competition_score']} "
    f"checked={result['checked_competition_count']} "
    f"candidates={result['candidate_count']} "
    f"recommended_league_id={league_id} "
    f"recommended_season={season_id} "
    f"warnings={len(result['warnings'])}"
)
if league_id != "none" and season_id != "none":
    print(
        "api_football_mapping_env "
        f"NUTMEG_API_FOOTBALL_MAPPING_LEAGUE_ID={league_id} "
        f"NUTMEG_API_FOOTBALL_MAPPING_SEASON={season_id}"
    )

for candidate in result.get("candidates", [])[: int(discovery_payload["max_competition_candidates"])]:
    season = candidate.get("recommended_season") or {}
    print(
        "api_football_candidate "
        f"league_id={candidate.get('provider_competition_id')} "
        f"name={candidate.get('name')!r} "
        f"country={candidate.get('country_name')!r} "
        f"score={candidate.get('score')} "
        f"season={season.get('provider_season_id', 'none')} "
...
record_provider_ops_run(
    base_url,
    admin_token=admin_token,
    run_name="api-football-discovery",
    ...
    summary_json={
        ...
        "recommended_league_id": league_id,
        "recommended_season": season_id,
        "warnings": len(result["warnings"]),
    },
    output_excerpt="api_football_competition_discovery_ok",
)
PY
REMOTE
