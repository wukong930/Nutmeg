...
PINNED: dict[str, dict] = {
    ...
    # ── 2026-09-18 横幅(1/51)· 日乙 ──
    # ⭐ 它是 09-11 **故意留下的 12 个**之一(见 `test_the_unanchored_j2_teams_were_not_guessed`)。
    #    当时的记录是「两条锚源都够不着」,并留了一句「哪天它们再上架、当场就有锚」——
    #    这次就是验证那句话:在售那场给出了第①档锚。
    # ⚠️ 用 `af_fixture` 而非 odds_snapshots:09-19 那场 Pinnacle 未开、该表 0 行。
    "藤枝MYFC": {          # 词典写「藤枝」—— 长短写法
        "en": "Fujieda MYFC",
        "anchor": ("Omiya Ardija", "away"),
        "af_fixture": 1606658,
        "date": "2026-09-19", "league": "JPN_J2",
        "already_ok": ("大宫松鼠RB", "Omiya Ardija"),
    },
...
def test_the_unanchored_j2_teams_were_not_guessed() -> None:
    ...
    ✅ **2026-09-18:`藤枝MYFC` 已补**(见 `PINNED`)—— 它再次上架,当场拿到第①档锚,
    正是上面那句「哪天它们再上架、当场就有锚」说的情况。**名单故意保持 12 个不变**:
    这条断言对「补没补」不敏感,留着它等于给已补的那条继续做「补错」看门狗。
    ⚠️ 同日复查过另外 11 个:AF 的 J2 覆盖已从「只到 2026-05-23」长到
    2026-08-08→09-20,但档案窗内日乙只有 8 场、涓流游标仍在 2021-12-12,
    逐个跑配对锚**全部落空** ⇒ 继续留着。
