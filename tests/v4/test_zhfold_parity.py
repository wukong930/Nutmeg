...
_KNOWN_DIVERGENCE: dict[str, str] = {
    ...
    # 2026-09-14 —— 亚冠精英比分锚定带进来的 2 条,与上面那串 SC 的分歧**逐字相同**
    # (服务端剥 SC、前端 SC 只在前缀组),不是新的一类。
    # ⚠️ 照 `Monagas SC` 立的规矩,钉之前量过它在遮什么:全词典折叠成
    #    ('al','duhail') / ('al','rayyan') 的**各只有它自己一个** ⇒ 无撞车,
    #    本次一条新撞车也没引入(全词典折叠撞车 62 处,与本次改动前一致)。
    "Al-Duhail SC": "SC:同上(2026-09-14 随亚冠精英比分锚定进词典)。已查无撞车",
    "Al-Rayyan SC": "SC:同上(同批)。已查无撞车",
    # (b) 记号只在前端词表里 —— 反方向
    ...
    # 2026-08-06 harvest 分支带进来的第 16 条。⭐ 这条是本护栏**第一次真的拦住东西**:
    # `test_zhfold_parity` 在那个分支的基线(d2b3950)上还不存在,所以分支自己看不见 ——
    # 两个分支各自全绿,合起来才红。语义冲突,git 不会报。
    #
    # 服务端**不剥**是对的:`_FOUNDING_YEAR = ^(0[0-9]|1[0-9]|[5-9][0-9])$` 只认 00–19/50–99,
    # 20–49 故意排除 —— 布雷斯特的 29 是**省号**(Finistère)不是建队年。前端那个无条件
    # `\\d{2}$` 才是更松的一边(它也剥掉 `Schalke 04` 的 04,那个确实是年份,所以两边同意)。
    "Stade Brestois 29": "29 是省号不是年份:服务端 _FOUNDING_YEAR 排除 20–49 ⇒ 保留",
